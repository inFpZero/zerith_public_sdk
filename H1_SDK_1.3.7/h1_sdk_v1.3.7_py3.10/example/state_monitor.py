    ...
    def run(self):
        while not self._stop.is_set():
            try:
                ...
                # 新增：手臂/头部末端实际和相对位姿（左右臂/头/相机）
                pose_keys = [
                    ("getHandRelative", "hand_relative"),
                    ("getHeadRelative", "head_relative"),
                    ("getHandCameraRelative", "hand_camera_relative"),
                    ("getHeadCameraRelative", "head_camera_relative"),
                ]
                # 需要 ArmAction.LEFT_ARM/RIGHT_ARM
                try:
                    left_val = ArmAction.LEFT_ARM
                    right_val = ArmAction.RIGHT_ARM
                except Exception:
                    left_val, right_val = 0, 1
                for method, key in pose_keys:
                    if hasattr(self.robot, method):
                        try:
                            if "Hand" in method:
                                left_ok, left_pose = getattr(self.robot, method)(left_val)
                                right_ok, right_pose = getattr(self.robot, method)(right_val)
                                pose_dict = {}
                                if left_ok:
                                    pose_dict["LEFT_ARM"] = _struct_to_dict(left_pose)
                                if right_ok:
                                    pose_dict["RIGHT_ARM"] = _struct_to_dict(right_pose)
                                if pose_dict:
                                    snapshot[key] = pose_dict
                            else:
                                ok, pose = getattr(self.robot, method)()
                                if ok:
                                    snapshot[key] = _struct_to_dict(pose)
                        except Exception as e:
...
def main():
    ...
    try:
        # 实例化机器人
        # robot = H1Robot("192.168.3.246")   #SDK运行环境为用户主机，无线连接方式，ip不固定
