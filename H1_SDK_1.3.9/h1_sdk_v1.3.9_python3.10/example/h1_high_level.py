...
def wait_mode(robot: H1Robot, target: MotorControlMode, timeout=5.0, interval=0.1):
    """
    轮询等待控制模式切换完成
    """
    t0 = time.time()
...
def ensure_high_level(robot: H1Robot):
    """
    确保进入 HIGH_LEVEL:
...
def chassis_test(robot: H1Robot):
    """
...
def gripper_test(robot: H1Robot):
    """
    抓手高层：与低层类似，位置 0/1
    """
...
def print_arm_target(robot: H1Robot):
    """
    打印 ArmEndPose: position[3], rotation[4](qx,qy,qz,qw)
    """
    ok, st = robot.getHandRelative(ArmAction.LEFT_ARM)
    if not ok:
        print("getHandRelative returned failure for LEFT_ARM"); return
