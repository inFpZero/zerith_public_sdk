...
float retreat = -0.2; //底盘后退速度 m/s
float stop = 0; //底盘停止速度 m/s
float left =0.15; //左转修改值
float right =-0.15; //右转修改值
//==============================================================腰部参数配置=====================================================
//腰部升降配置
...
void ChassisTest_advance(H1Robot& robot){
    ...
    for(int i = 0; i < 30; ++i) {
        ...
        robot.getChassisSpeedState(speed_actual, speed_algo);
        //控制值和实际值
        std::cout << "控制值: 线速度:" << speed_x << " m/s, 角速度:" << speed_y << " rad/s" << std::endl;
        //电机实际转速，解算速度
        std::cout << "解算值: 线速度:" << speed_algo[0] << " m/s, 角速度:" << speed_algo[1] << " rad/s" << std::endl;
        std::cout << "实际转速: 左轮:" << speed_actual[0] << " rad/s, 右轮:" << speed_actual[1] << " rad/s" << std::endl;
        std::cout << std::endl;
    }
...
void HighLevelControlTest_arm(H1Robot& robot) {
    ...
    for (const auto& armPose : armPoses) {  
        std::cout << "执行ArmPose: {" << armPose.x << ", " << armPose.y << ", " 
         ...
         auto quatPose = robot.armPoseToArmEndPose(armPose); // 默认姿态不变
        robot.setArmMove_high(ArmAction::RIGHT_ARM, quatPose,0,0,10);//默认速度和时间
...
void HighLevelControlTest_arm1(H1Robot& robot) {
    ...
    std::cout << "Start HighLevelControlTest_arm" << std::endl;
    ArmPose armPose{0.2, -0.2, 0, 0, 0, 0};
    auto quatPose = robot.armPoseToArmEndPose(armPose); // 默认姿态不变

    robot.setArmMove_high(ArmAction::LEFT_ARM, quatPose);//默认速度和时间
    std::this_thread::sleep_for(std::chrono::milliseconds(5000));
    //在此处添加复杂动作测试代码
...
int main() {
    ...
    while(true) {
        ...
        // HighLevelControlTest_arm1(robot);
