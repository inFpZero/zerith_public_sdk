...
public:
    ...
    ArmEndPose left_hand_actual{};                     // 左臂实际位置
    ArmEndPose right_hand_actual{};                    // 右臂实际位置
    ArmEndPose head_actual{};                         // 头部(腰部)实际位置
    ArmEndPose left_hand_relative{};                   // 左臂相对位置
    ArmEndPose right_hand_relative{};                  // 右臂相对位置
    ArmEndPose head_relative{};                       // 头部(腰部)相对位置
    ArmEndPose left_camera_actual{};                  // 左臂相机实际位置
    ArmEndPose right_camera_actual{};                 // 右臂相机实际位置
    ArmEndPose head_camera_actual{};                  // 头部(腰部)相机实际位置
    ArmEndPose left_camera_relative{};                // 左臂相机相对位置
    ArmEndPose right_camera_relative{};               // 右臂相机相对位置
    ArmEndPose head_camera_relative{};                // 头部(腰部)相机相对位置
...
private:
    ...
    void Left_Hand_Actual_ZCM(const zcm::ReceiveBuffer* rbuf, 
                            const std::string& chan,
                            const ArmEndPose* msg);
    void Right_Hand_Actual_ZCM(const zcm::ReceiveBuffer* rbuf, 
                            const std::string& chan,
                            const ArmEndPose* msg);
    void Head_Actual_ZCM(const zcm::ReceiveBuffer* rbuf, 
                            const std::string& chan,
                            const ArmEndPose* msg);
    void Left_Hand_Relative_ZCM(const zcm::ReceiveBuffer* rbuf, 
                            const std::string& chan,
                            const ArmEndPose* msg);
    void Right_Hand_Relative_ZCM(const zcm::ReceiveBuffer* rbuf,
                            const std::string& chan,
                            const ArmEndPose* msg);
    void Head_Relative_ZCM(const zcm::ReceiveBuffer* rbuf, 
                            const std::string& chan,
                            const ArmEndPose* msg);
    void Left_Camera_Actual_ZCM(const zcm::ReceiveBuffer* rbuf, 
                            const std::string& chan,
                            const ArmEndPose* msg);
    void Right_Camera_Actual_ZCM(const zcm::ReceiveBuffer* rbuf, 
                            const std::string& chan,
                            const ArmEndPose* msg);
    void Head_Camera_Actual_ZCM(const zcm::ReceiveBuffer* rbuf, 
                            const std::string& chan,
                            const ArmEndPose* msg);
    void Left_Camera_Relative_ZCM(const zcm::ReceiveBuffer* rbuf, 
                            const std::string& chan,
                            const ArmEndPose* msg);
    void Right_Camera_Relative_ZCM(const zcm::ReceiveBuffer* rbuf, 
                            const std::string& chan,
                            const ArmEndPose* msg);
    void Head_Camera_Relative_ZCM(const zcm::ReceiveBuffer* rbuf, 
                            const std::string& chan,
                            const ArmEndPose* msg);
