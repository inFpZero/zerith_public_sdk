...
public:
    ...
    //设置灵巧手控制
    bool setHand_low(ArmAction arm, const HandControl& control);
    ...
    // // 高阶控制命令类型
    // struct HighLevelControl
    // {
    //     // 控制类型：0=连续控制, 1=Move模式，
    //     int32_t control_type;

    //     // 控制部位：0=左手, 1=右手, 2=腰部
    //     int32_t control_part;

    //     // 腰部三个电机位置
    //     float waist_position[3];
        
    //     // 右手位姿：位置(3) + 四元数(4) = 7个float
    //     // 格式：[x, y, z, qx, qy, qz, qw]
    //     float hand_pose[7];
        
    //     // 末端执行器速度 (m/s)，可选，默认值 0.0 表示使用默认速度
    //     float eef_velocity;
        
    //     // 末端执行器加速度 (m/s^2)，可选，默认值 0.0 表示使用默认加速度
    //     float eef_acceleration;
        
    //     // 是否阻塞执行，true=阻塞，false=非阻塞
    //     boolean block;
        
    //     // 运动持续时间 (秒)，可选，默认值 0.0 表示自动计算
    //     float duration;
    // }

    // 设置手臂末端一键运动,控制速度或时间
    // eef_velocity: 末端执行器速度 (m/s)，0表示使用默认速度
    // eef_acceleration: 末端执行器加速度 (m/s^2)，0表示使用默认加速度
    // duration: 运动持续时间 (秒)，0表示自动计算
    //bool block: 是否阻塞后续动作执行，true=阻塞，false=非阻塞，默认false
    bool setArmMove_high(ArmAction arm, const ArmEndPose& action, float eef_velocity = 0.0f, float eef_acceleration = 0.0f, float duration = 0.0f, bool block = false);

    // 设置夹爪末端姿态，实际与低层控制相同
    bool setGripper_high(EtherCAT_Motor_Index gripper_id, const Motor_Control& control, bool is_hold_torque = true);

    //设置灵巧手控制
    bool setHand_high(ArmAction arm, const HandControl& control);
    ...
    // ===== 不区分控制模式的接口 =====
    // 清零力传感器数据,0左传感器，1右传感器
    bool resetForceSensorData(const int sensor_id);
    ...
    // 获取手臂和头部末端实际和相对位姿
    //相对电机零位的坐标系（relative）
    bool getHandRelative(ArmAction arm, ArmEndPose& state);
    bool getHeadRelative(ArmEndPose& state);
    
    bool getHandCameraRelative(ArmAction arm, ArmEndPose& state);
    bool getHeadCameraRelative(ArmEndPose& state);
    // 获取夹爪状态
    ...
    // 获取灵巧手状态
    bool getHandState(ArmAction arm, HandState& state);
    // 获取固定杆状态
    ...
    //具体参数含义如上述
    // 获取高级控制状态反馈
    bool getHighLevelState(HighLevelState& state);
    ...
    // 获取 IMU 状态
    // state.rpy[0~2]       : 滚转、俯仰、偏航角 (roll, pitch, yaw)，单位 rad
    // state.omega[0~2]     : 三轴角速度，单位 rad/s
    // state.accel[0~2]     : 三轴线加速度，单位 m/s^2
    // state.quat[0~3]      : 四元数 (w, x, y, z)，与底层 IMU 协议一致
    // state.error_flag     : 错误标志，0 正常，其他 异常（未连接/读失败/超时）
    // 返回值：true - 成功，false - 失败（未连接）
    bool getIMU_State(ImuData& state);


    //获取力传感器状态
    bool getForceSensorState(ForceSensorState& state);
    //获取电源充电和放电状态
    ...
    // 获取当前初始化状态
    // 返回值：0 - 未初始化，1 - 初始化中，2 - 已初始化, 3 - 反初始化中，4 - 已反初始化
...
private:
    ...
    HardwareVersionInfo hw_version_info_;   // 新增成员
