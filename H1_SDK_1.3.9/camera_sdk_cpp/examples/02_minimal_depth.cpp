...
int main() {
  ...
  CameraClient client(GRPC_TARGET, true);
  client.start();

  const std::string window_name = "Depth Viewer";
  std::pair<int, int> window_size = {-1, -1};
  while (true) {
    ...
    if (depth_data) {
      ...
      std::pair<int, int> image_size = {depth_viz.cols, depth_viz.rows};
      if (window_size.first < 0) {
        cv::namedWindow(window_name, cv::WINDOW_NORMAL);
      }
      if (window_size != image_size) {
        cv::resizeWindow(window_name, depth_viz.cols, depth_viz.rows);
        window_size = image_size;
      }
      cv::imshow(window_name, depth_viz);
    }
