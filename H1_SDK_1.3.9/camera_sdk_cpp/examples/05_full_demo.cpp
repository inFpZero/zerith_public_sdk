...
int main(int argc, char** argv) {
  // 1. 初始化客户端
  CameraClient client(Config::GRPC_TARGET, true);
  ...
  std::map<std::string, std::pair<int, int>> window_sizes;
  ...
  auto show_image = [&window_sizes](const std::string& window_name, const cv::Mat& image) {
    std::pair<int, int> image_size = {image.cols, image.rows};
    auto it = window_sizes.find(window_name);
    if (it == window_sizes.end()) {
      cv::namedWindow(window_name, cv::WINDOW_NORMAL);
      it = window_sizes.emplace(window_name, std::make_pair(-1, -1)).first;
    }
    if (it->second != image_size) {
  ...
  try {
    while (true) {
      ...
      for (const auto& name : rgb_cameras) {
        ...
        show_image("RGB - " + name, image_display);
      ...
      for (const auto& name : depth_cameras) {
        ...
        if (depth_data) {
          ...
          show_image("Depth - " + name, depth_display);
          any_window_updated = true;
        }
      }
